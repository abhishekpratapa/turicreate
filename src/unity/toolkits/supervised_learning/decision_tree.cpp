...
std::shared_ptr<MLModelWrapper> decision_tree_regression::export_to_coreml(const std::string& filename) {
  ...
  return this->_export_xgboost_model(filename, false, true, context);
}
...
std::shared_ptr<MLModelWrapper> decision_tree_classifier::export_to_coreml(const std::string& filename) {
